...
#    feature_dict['src_has_description'] = 0
    ...
    if tw['user']['description'] != None:
        feature_dict['src_description'] = help_prep_functions.text_sumw2v(
                                                     tw['user']['description'],
                                                     avg=True)
    feature_dict['src_usr_favourites_count'] = int(
                                                tw['user']['favourites_count'])
    feature_dict['src_geo_enabled'] = int(tw['user']['geo_enabled'])
    if feature_dict['src_num_friends'] != 0:
        feature_dict['src_follow_ratio'] = float(
                                    feature_dict['src_num_followers'])/float(
                                    feature_dict['src_num_friends'])
    else:
        feature_dict['src_follow_ratio'] = float(
                                        feature_dict['src_num_followers'])/1.0

    acc_create_date = datetime.strptime(tw['user']['created_at'],
                                        '%a %b %d %H:%M:%S +0000 %Y')
    ...
    q = 0
#%%
    # extract autostance stance labels were extracted using the branch
    # LSTM model trained on RumourEval data
    autostance_path = "data/predicted_stance.txt"
    with open(autostance_path, 'rb') as f:
        autostance = pickle.load(f)
    if tw['id_str'] in autostance.keys():
        # print 'Found! source'
        if autostance[tw['id_str']] == 0:
            s = s+1
        elif autostance[tw['id_str']] == 2:
            d = d+1
        elif autostance[tw['id_str']] == 3:
            q = q+1
    for repl in conversation['replies']:
        if repl['id_str'] in autostance.keys():
            # print 'Found! reply'
            if autostance[repl['id_str']] == 0:
                s = s+1
            elif autostance[repl['id_str']] == 2:
                d = d+1
            elif autostance[repl['id_str']] == 3:
                q = q+1
    ...
    feature_dict['Stance_Ratio_Deny'] = deny_stanceratio
    feature_dict['Stance_Ratio_Question'] = question_stanceratio
    feature_dict['Stance_Ratio_Support'] = support_stanceratio
    return feature_dict
...
def extract_thread_features_incl_response(conversation):
    ...
    for tw in conversation['replies']:
        ...
        acc_create_date = datetime.strptime(tw['user']['created_at'],
                                            '%a %b %d %H:%M:%S +0000 %Y')
        tw_date = datetime.strptime(tw['created_at'],
                                    '%a %b %d %H:%M:%S +0000 %Y')
        feature_dict['account_age'] = (tw_date - acc_create_date).days
        feature_dict['src_has_coordinates'] = 0
        if tw['coordinates'] != None:
